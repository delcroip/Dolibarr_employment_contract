...
$langs->load("Hrsalarymethod_class");
...
$upload_dir = $conf->emcontract->dir_output.'/Hrsalarymethod/'.dol_sanitizeFileName($object->ref);


 // uncomment to avoid resubmision
//if(isset( $_SESSION['Hrsalarymethod_class'][$tms]))
//{
...
//}
 $tms= time();
 $_SESSION['Hrsalarymethod_class'][$tms]= array();
...
}else if (($action == 'add') || ($action == 'update' && ($id>0 || !empty($ref))))
{
		...
		$object->ct_custom_fields_1_desc=GETPOST("Ctcustomfields1desc");
		$object->ct_custom_fields_2_desc=GETPOST("Ctcustomfields2desc");
		$object->c_custom_fields_1_desc=GETPOST("Ccustomfields1desc");
		$object->c_custom_fields_2_desc=GETPOST("Ccustomfields2desc");
...
if(isset( $_SESSION['Hrsalarymethod_class'][$tms]))
{
    unset($_SESSION['Hrsalarymethod_class'][$tms]);
}
...
switch ($action) {
   ...
   case "delete";
        if( $action=='delete' && ($id>0 || $ref!="")){
         $ret=$form->form_confirm($_SERVER["PHP_SELF"].'?action=confirm_delete&id='.$id,$langs->trans("DeleteHrsalarymethod"),$langs->trans("ConfirmDelete"),"confirm_delete", '', 0, 1);
         if ($ret == 'html') print '<br />';
         //to have the object to be deleted in the background\
        }
    case "view":
    {
        //print_fiche_titre($langs->trans('Hrsalarymethod'));
        	// tabs
        if($edit==0 && $new==0){ //show tabs
            $head=Hrsalarymethod_prepare_head($object);
            dol_fiche_head($head,'card',$langs->trans("Hrsalarymethod"),0,'emcontract@emcontract');            
        }
	print '<br>';
        if($edit==1){
            if($new==1){
...
// show the field ct_custom_fields_1_desc

		print "<td class='fieldrequired'>".$langs->trans('Ctcustomfields1desc')." </td><td>";
		if($edit==1){
			print '<input type="text" value="'.$object->ct_custom_fields_1_desc.'" name="Ctcustomfields1desc">';
		}else{
			print $object->ct_custom_fields_1_desc;
		}
		print "</td>";

// show the field ct_custom_fields_2_desc

		print "<td>".$langs->trans('Ctcustomfields2desc')." </td><td>";
		if($edit==1){
			print '<input type="text" value="'.$object->ct_custom_fields_2_desc.'" name="Ctcustomfields2desc">';
		}else{
			print $object->ct_custom_fields_2_desc;
		}
		print "</td>";
		print "\n</tr>\n";
		print "<tr>\n";

// show the field c_custom_fields_1_desc

		print "<td>".$langs->trans('Ccustomfields1desc')." </td><td>";
		if($edit==1){
			print '<input type="text" value="'.$object->c_custom_fields_1_desc.'" name="Ccustomfields1desc">';
		}else{
			print $object->c_custom_fields_1_desc;
		}
		print "</td>";

// show the field c_custom_fields_2_desc

		print "<td>".$langs->trans('Ccustomfields2desc')." </td><td>";
		if($edit==1){
			print '<input type="text" value="'.$object->c_custom_fields_2_desc.'" name="Ccustomfields2desc">';
		}else{
			print $object->c_custom_fields_2_desc;
		}
		print "</td>";
		print "\n</tr>\n";
...
//FIXME DELETE
        ...
        //print_fiche_titre($langs->trans('Hrsalarymethod'));
        $head=Hrsalarymethod_prepare_head($object);
        dol_fiche_head($head,'info',$langs->trans("Hrsalarymethod"),0,'emcontract@emcontract');            
	...
	$object->fetch_thirdparty();
        ...
        //print_fiche_titre($langs->trans('Hrsalarymethod'));
        $head=Hrsalarymethod_prepare_head($object);
        dol_fiche_head($head,'documents',$langs->trans("Hrsalarymethod"),0,'emcontract@emcontract');            
    ...
    $sql.= " FROM ".MAIN_DB_PREFIX."hr_salary_method as t";
...
function Hrsalarymethod_prepare_head($object)
{
