...
dol_include_once('/emcontract/class/hrjobskills.class.php');
...
$upload_dir = $conf->emcontract->dir_output.'/Hrjobskills/'.dol_sanitizeFileName($object->ref);


 // uncomment to avoid resubmision
//if(isset( $_SESSION['Hrjobskills_class'][$tms]))
//{
...
//}
 $tms= time();
 $_SESSION['Hrjobskills_class'][$tms]= array();
...
if(isset( $_SESSION['Hrjobskills_class'][$tms]))
{
    unset($_SESSION['Hrjobskills_class'][$tms]);
}
...
switch ($action) {
   ...
   case "delete";
        if( $action=='delete' && ($id>0 || $ref!="")){
         $ret=$form->form_confirm($_SERVER["PHP_SELF"].'?action=confirm_delete&id='.$id,$langs->trans("DeleteHrjobskills"),$langs->trans("ConfirmDelete"),"confirm_delete", '', 0, 1);
         if ($ret == 'html') print '<br />';
         //to have the object to be deleted in the background\
        }
    case "view":
    {
        //print_fiche_titre($langs->trans('Hrjobskills'));
        	// tabs
        if($edit==0 && $new==0){ //show tabs
            $head=Hrjobskills_prepare_head($object);
            dol_fiche_head($head,'card',$langs->trans("Hrjobskills"),0,'emcontract@emcontract');            
        }
	print '<br>';
        if($edit==1){
            if($new==1){
...
//FIXME DELETE
        ...
        //print_fiche_titre($langs->trans('Hrjobskills'));
        $head=Hrjobskills_prepare_head($object);
        dol_fiche_head($head,'info',$langs->trans("Hrjobskills"),0,'emcontract@emcontract');            
	...
	$object->fetch_thirdparty();
        ...
        //print_fiche_titre($langs->trans('Hrjobskills'));
        $head=Hrjobskills_prepare_head($object);
        dol_fiche_head($head,'documents',$langs->trans("Hrjobskills"),0,'emcontract@emcontract');            
    ...
    $sql.= " FROM ".MAIN_DB_PREFIX."hr_job_skills as t";
...
function Hrjobskills_prepare_head($object)
{
