...
dol_include_once('/emcontract/class/hrcontractevent.class.php');
...
$langs->load("Hrcontractevent_class");
...
$upload_dir = $conf->emcontract->dir_output.'/Hrcontractevent/'.dol_sanitizeFileName($object->ref);


 // uncomment to avoid resubmision
//if(isset( $_SESSION['Hrcontractevent_class'][$tms]))
//{
...
//}
 $tms= time();
 $_SESSION['Hrcontractevent_class'][$tms]= array();
...
}else if (($action == 'add') || ($action == 'update' && ($id>0 || !empty($ref))))
{
		...
		$object->date_start=dol_mktime(0, 0, 0,'Datestartmonth','Datestartday','Datestartyear');
		$object->date_stop=dol_mktime(0, 0, 0,'Datestopmonth','Datestopday','Datestopyear');
...
if(isset( $_SESSION['Hrcontractevent_class'][$tms]))
{
    unset($_SESSION['Hrcontractevent_class'][$tms]);
}
...
switch ($action) {
   ...
   case "delete";
        if( $action=='delete' && ($id>0 || $ref!="")){
         $ret=$form->form_confirm($_SERVER["PHP_SELF"].'?action=confirm_delete&id='.$id,$langs->trans("DeleteHrcontractevent"),$langs->trans("ConfirmDelete"),"confirm_delete", '', 0, 1);
         if ($ret == 'html') print '<br />';
         //to have the object to be deleted in the background\
        }
    case "view":
    {
        //print_fiche_titre($langs->trans('Hrcontractevent'));
        	// tabs
        if($edit==0 && $new==0){ //show tabs
            $head=Hrcontractevent_prepare_head($object);
            dol_fiche_head($head,'card',$langs->trans("Hrcontractevent"),0,'emcontract@emcontract');            
        }
	print '<br>';
        if($edit==1){
            if($new==1){
...
// show the field date_stop

		print "<td class='fieldrequired'>".$langs->trans('Datestop')." </td><td>";
		if($edit==1){
		if($new==1){
			print $form->select_date(-1,'Datestop');
		}else{
			print $form->select_date($object->date_stop,'Datestop');
		}
		}else{
			print dol_print_date($object->date_stop,'day');
		}
		print "</td>";
		print "\n</tr>\n";
		print "<tr>\n";
...
//FIXME DELETE
        ...
        //print_fiche_titre($langs->trans('Hrcontractevent'));
        $head=Hrcontractevent_prepare_head($object);
        dol_fiche_head($head,'info',$langs->trans("Hrcontractevent"),0,'emcontract@emcontract');            
	...
	$object->fetch_thirdparty();
        ...
        //print_fiche_titre($langs->trans('Hrcontractevent'));
        $head=Hrcontractevent_prepare_head($object);
        dol_fiche_head($head,'documents',$langs->trans("Hrcontractevent"),0,'emcontract@emcontract');            
    ...
    $sql.= " FROM ".MAIN_DB_PREFIX."hr_contract_event as t";
...
function Hrcontractevent_prepare_head($object)
{
