...
/**
 *  \file       dev/hrskills/hrskill.class.php
 ...
 */
class Hrskill extends CommonObject
{
	var $db;							//!< To store db handler
	var $error;							//!< To return error code (or message)
	var $errors=array();				//!< To return several error codes (or messages)
	var $element='hrskill';			//!< Id that identify managed objects
	var $table_element='hr_skill';		//!< Name of table without prefix where object is stored
    ...
    function getNomUrl($withpicto=0)
    {
        ...
        if(isset($this->ref))
            $ref=$this->ref;
        if($id)
            $lien = '<a href="'.DOL_URL_ROOT.'/emcontract/hrskill.php?id='.$id.'&action=view">';
    	else if ($ref)
            $lien = '<a href="'.DOL_URL_ROOT.'/emcontract/hrskill.php?ref='.$ref.'&action=view">';
    	else
            return "Error";
        $lienfin='</a>';
